...
> 수강료가 없으면 위 가중합에서 +30 (가장 큰 키)과 현장/불라방 판정(`enrollments.mode`)이 통째로 사라지고,
> 남는 신호는 시간대·트랙·강좌유형·강사명·수강기간뿐이라 `top1 ≥ 70` 자동 확정 기준을 그대로 쓸 수 없다.
> 선택지는 세 가지이고 **아직 Alan 확정 전이다 (미확정 3 과 함께 정한다)**:
> ① 수강료가 있는 반만 자동 확정하고 없으면 후보를 학생에게 보여 주고 고르게 한다
> ② 자동 확정 기준선을 수강료 없이도 성립하도록 다시 잡는다
...
2. ~~**수강료 실제 금액**~~ → **해결 (2026-09-16 Alan): 수강료는 넣지 않아도 된다.** 결제·등록은 YBM 에서만 하고
   ...
   금액을 임의로 지어내지 말 것. 남은 것은 **수강료 없이 OCR 자동 등업을 어떻게 판정할지**다 (위 "수강료를 최우선 키로 쓰는 이유" 참고, 미확정 3 과 같이 정한다).
...
- **현장/불라방 구분**: `enrollments.mode` (onsite | live). OCR 은 수강료가 `tuition` 이면 onsite, `live_tuition` 이면 live 로 판정.
  단 수강료는 선택이라 **반에 금액이 없으면 이 판정이 불가능하다** (2026-09-16). 그때 어떻게 할지는 미확정 2·3 에서 같이 정한다
