...
**담당 강사 일괄 지정** (`/admin/sections`, 2026-09-16 Alan 요청 — "반편성에서 이혜영·이영수 제대로 적용해줘")
- 반이 한 달에 70개 안팎이라 반 상세에 하나씩 들어가 바꾸지 않는다. 개설 반 목록 아래 카드에서
  반을 여러 개 고르고 강사를 정해 한 번에 바꾼다 (`assignInstructor`, 한 번에 300개까지).
- **어느 반을 누가 맡는지는 화면이 정해 주지 않는다.** 그 달 편성표가 정하고(이혜영 LC · 이영수 RC),
  같은 반도 트랙마다 과목이 달라서(650A = 월수금 RC · 화목금 LC) 코드가 짐작할 근거가 없다 — 미확정 1 의 마지막 항목.
  강사를 자동으로 채워 넣지 말 것.
- 담당으로 둘 수 있는 것은 **강사·관리자 계정뿐**이다 (서버가 대상 계정의 등급을 다시 본다). 조교는 담당 강사가 될 수 없다.
