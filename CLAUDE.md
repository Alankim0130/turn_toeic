...
- 수강료는 위쪽 기본값을 강좌 카드마다 덮어쓸 수 있다. **불라방은 별도 반이 아니라 같은 반의 수강 방식**이라
  반마다 현장(`tuition`)·불라방(`live_tuition`) 두 금액을 함께 넣는다 (2026-09-16 Alan 확정)
...
1. **시간대·반 목록** — 어떤 시간대에 어떤 반이 열리는지. Alan이 정리해서 전달 예정.
   ...
   `time_block` 은 일괄 개설에서 시간표 슬롯 라벨을 그대로 쓰므로 해결됐고, **매달 반 수강료 금액만 여전히 미확정.**
