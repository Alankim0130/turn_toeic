...
- **`LC 교재`(A반/B반)는 시간대 · 트랙마다 고른다** (2026-09-16 편성표로 정정). 120분 반은 A 시간과 B 시간을 이어 듣는데,
  **LC 를 듣는 시간이 트랙마다 달라서** 교재도 트랙마다 다르다. 반의 `book_set` = 그 반의 LC 시간이 쓰는 세트.
  9월: 650 월수금 B · 화목금 A / 750 월수금 A · 화목금 B / 850 12:30 A · 13:50 B (10월은 A·B 가 서로 바뀐다).
  스파르타 반은 교재를 두지 않는다 — 함께 듣는 점수보장반의 교재를 쓴다. 학생 화면의 "내 반 교재"와 수업일 달력이 이 값을 읽는다 (도메인 규칙 7)
- 이미 있는 (강좌·트랙·시간대) 조합은 `있음` 으로 잠기고 저장할 때 다시 세어 건너뛴다 — 같은 반이 두 번 생기지 않는다

**반 권한 — 스파르타반** (2026-09-16 Alan 요청 "반 권한설정에 유의", 마이그레이션 20260916180000)
- 스파르타반은 두 레벨을 함께 듣는다 (650+ 중급속성 = 650 LC/RC + 850 LC/RC). 그래서 **스파르타 반에 배정된 학생은
  같은 기수 · 같은 트랙에서 시간이 겹치는 포함 레벨(자기 레벨 + `includes_levels`)의 점수보장반 권한을 함께 받는다.**
  예) 스파르타 650 월수금 10:00~13:40 → 650 월수금 10:00~12:10 + 850 월수금 12:30~13:40. 850 13:50 반·화목금 반·750 반은 **열리지 않는다.**
  스파르타 750 260분(10:00~15:00) → 750 10:00~12:10 + 850 12:30~13:40 + 850 13:50~15:00.
- 판정은 DB 한곳: `private.section_includes(스파르타 반, 대상 반)` → `private.has_section_access` 가 쓴다.
  그래서 수업일 · 다시보기 · 불라방 링크(전부 RLS) 가 자동으로 따라오고, **녹화본은 점수보장반에 한 번만 올린다.**
  화면이 레벨·교재를 정할 때는 `public.my_section_ids()`(접근 가능한 반 id)를, 관리자 표시는 `public.term_section_includes(기수)` 를 쓴다.
  **화면에서 포함 관계를 따로 계산하지 말 것.**
- 스파르타 학생 화면: LC 음원은 포함 레벨이 모두 열린다(650 + 850). 내 시간표는 같은 날·같은 트랙에 실제 수업(점수보장반)이 있으면
  스파르타 반 줄을 숨긴다. 반 편성 목록·반 상세에 "이 반 학생에게 함께 열리는 반" 이 보인다.
- 반 배정 · 승인 화면의 반 이름은 시간 컬럼이 없으면 `time_block` 을 붙인다 (`sectionSummary`) — 같은 강좌의 오전·저녁반을 구분한다
...
-- private.section_includes(스파르타 반, 대상 반) → has_section_access. public.my_section_ids() · public.term_section_includes(기수)
...
create table class_sections (              -- 트랙 단위 분반
  ...
  book_set      text,                      -- LC 교재 세트 'A'|'B' (마이그레이션 20260916160000).
                                           -- 시간대마다 다르고 달마다 뒤바뀐다. null 이면 미지정(달 홀짝으로 짐작)
...
1. **시간대·반 목록** — 어떤 시간대에 어떤 반이 열리는지. Alan이 정리해서 전달 예정.
   ...
   **평달 (9·10월)** — 60분 단위로 쪼개 적은 **강사 배치표**다 (강의실 701 · 301). 학생이 등록하는 단위는 두 타임을 붙인 120분(850 은 140분)이다.
   ...
   - ~~문법반은 다루지 않는다~~ → **정정 (2026-09-16 Alan 재확인): 12:30·13:50 의 "문풀" 은 문법이 아니라 문제풀이 = 850+ 문제마스터다.**
     ...
     그래서 850 은 평달에도 열고 `timetable_slots` 의 850 `12:30~13:40 · 13:50~15:00` 이 맞다 (70분 반 두 개 — 스파르타 190분이 앞 70분만 듣는다).
   ...
   - **2026년 9월 반은 이 편성표대로 18개 개설했다** (2026-09-16): 650·750 × 10:00~12:10 · 18:30~20:40, 850 × 12:30~13:40 · 13:50~15:00,
