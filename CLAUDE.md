...
- **랜딩 수강후기** (2026-09-17 Alan 제공): YBM 공식 수강후기 화면을 찍은 캡쳐를 **옆으로 넘겨 보는 카드**로 두고(통계 다음),
  누르면 **팝업으로 전체**가 열린다 (`Reviews`). 캡쳐가 세로로 길어 카드에는 윗부분만 보이기 때문이다.
  팝업은 ← → 로 다음 후기, ESC·배경·닫기로 닫고, 열려 있는 동안 뒤 화면 스크롤을 막는다.
  목록은 `site.reviews` 한곳 — 후기를 더하려면 `public/reviews/` 에 이미지를 넣고 한 줄 더한다.
  **본문을 옮겨 적지 않는다** — 캡쳐가 원문이고 코드에는 제목·강좌·작성자·날짜만 둔다. 작성자 아이디는 YBM 이 가린 그대로 쓴다.
  **카드 캡션에 제목을 또 적지 말 것** — 캡쳐 맨 위에 제목이 이미 있어 같은 말이 두 번 나온다 (`alt` 와 팝업에는 남긴다).
  이미지는 WebP 폭 900px 으로 줄여 넣었고(2.9MB → 435KB) `unoptimized` 라 Vercel 이미지 변환(과금)을 타지 않는다.
