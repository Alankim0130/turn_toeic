...
1. **시간대·반 목록** — 어떤 시간대에 어떤 반이 열리는지. Alan이 정리해서 전달 예정.
   ...
   → **방학달은 시간표가 다르다** (2026-09-16 Alan). **7·8월과 1·2월**은 평달과 시간대가 조금 다르고, 위 시간은 **평달 기준**이다.
   `timetable_slots` 에는 달·기간 구분이 없어 레벨당 한 세트만 들어가고, 그 한 세트가 랜딩 시간표와 **반 일괄 개설 표**에 같이 쓰인다.
   그래서 방학달 기수를 편성하면 표에 평달 시간대가 그대로 뜨고, 그대로 개설하면 `time_block` 이 틀린 값으로 박혀 OCR 시간대 대조(+20점)가 어긋난다.
   **방학달(7·8·1·2월) 반은 Alan 에게 그 달 레벨별 실제 시간대를 받기 전까지 만들지 말 것.**
   받은 뒤 구현 방식(시간대에 적용 기간을 붙일지, 일괄 개설 표에서 그 달만 고칠지)도 Alan 에게 확인하고 정한다.
