...
| `주5일 (월18회) 월수금(현강)+화목금(인강)` | 주5일 | 월수금 + 화목금 | 현장 (화목금 인강 — 미확정 2-2) |
...
- 120분 등록생은 수강증 시간과 같은 `time_block` 의 반과 대조한다. **60분 등록생을 어느 반에 배정할지는 아직 없다 — 미확정 2-2.**
...
2-1. ~~**현장/불라방(`enrollments.mode`)을 무엇으로 판정할지**~~ → **해결 (2026-09-16 Alan): 수강증의 `라이브방송` 표기로 판정한다** (선택지 b).
   ...
   **남은 것 하나**: 승인 화면(`/admin/verifications/[id]`)의 현장/불라방 라디오가 **현장으로 미리 선택**돼 있다(`defaultChecked`).
   그냥 승인하면 불라방 학생이 조용히 onsite 로 박히고 `/my/textbook` 이 막힌다(`queries.ts` 가 `mode = 'live'` 로 거른다).
   **기본값을 없애고 스태프가 반드시 고르게 할지**를 Alan 이 정한다 (승인 1건마다 클릭이 하나 늘어난다). OCR 이 생기면 수강증 표기로 미리 골라 둘 수 있다.
2-2. **수강증 표기 규칙에서 남은 질문** (2026-09-16 Alan 이 표기를 알려 주며 생겼다. OCR·배정을 구현하기 전에 확인)
   - **60분 등록생을 어느 반에 배정할지.** 지금 650·750 반은 120분(`10:00~12:10`) 한 반이고 60분짜리 반은 없다.
     `10:00~11:00` 등록생을 (가) 120분 반에 그대로 배정 — 다시보기·불라방이 120분 전체로 열린다 /
     (나) 60분 반(`10:00~11:00` · `11:10~12:10`)을 따로 연다 — 한 달 반 수가 늘어난다 / (다) 120분 반에 배정하고 등록에 "60분" 만 표시한다.
   - **850 은 한 타임이 70분이다** (`12:30~13:40` · `13:50~15:00`, 반도 두 개). `12:30~15:00` 수강증이면 850 반 두 개에 모두 배정하는지.
   - **`주5일 (월18회) 월수금(현강)+화목금(인강)` 의 화목금 인강.** `라이브방송` 이 없으니 규칙대로면 두 반 모두 현장(onsite)이다.
     화목금 저녁반이 원래 인강으로 운영되는 반이라 학생별 구분이 아니면 그대로 두고, 불라방처럼 다뤄야 하면(교재주문 등) `live` 로 잡는다.
