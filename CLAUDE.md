...
- 시간대 행은 랜딩 시간표(`timetable_slots`)에서 읽고, 강좌는 `courses.target_score` 로 자기 레벨의 시간대만 본다.
...
- **시간대마다 `LC 교재`(A반/B반/미지정)를 고른다.** 같은 시간대의 월수금·화목금 두 반은 같은 교재를 쓴다.
  학생 화면의 "내 반 교재"와 수업일 달력이 이 값을 읽는다 (도메인 규칙 7)
...
1. **시간대·반 목록** — 어떤 시간대에 어떤 반이 열리는지. Alan이 정리해서 전달 예정.
   ...
   **평달 (9·10월)** — 60분 단위로 쪼개 적은 **강사 배치표**다. 학생이 등록하는 단위는 두 타임을 붙인 120분이다.

   | 시간 | 9월 | 10월 |
   |---|---|---|
   | 10:00–11:00 | 650A · 750A | 650B · 750B |
   | 11:10–12:10 | 650B · 750B | 650A · 750A |
   | 12:30–13:40 (70분) | 문법A | 문법B |
   | 13:50–15:00 (70분) | 문법B | 문법A |
   ...
   - 각 칸은 트랙마다 과목이 다르다. 예) 9월 650A = **월수금 RC · 화목금 LC**, 650B = 월수금 LC · 화목금 RC.
     650과 750은 같은 시간에 서로 반대 과목이다 (이혜영 LC · 이영수 RC 가 한 시간씩 나눠 맡는다).
   - **시간대가 과목을 고정하고, 교재 세트 A/B 만 달마다 뒤바뀐다.** 9월 10:00 이 A면 10월 10:00 은 B.
   - **문법반은 이 사이트에서 다루지 않는다** (2026-09-16 Alan 확정).
   ...
   **확정된 것 (2026-09-16 Alan)**
   ...
   - 이 사이트에서 반으로 관리할 강좌: **650 · 750 · 850 · 스파르타반**. 문법은 제외.
   ...
   **아직 확정 안 된 것**
   - **850 의 평달 시간대.** `timetable_slots` 에 `850 12:30~13:40 · 13:50~15:00` 이 들어가 있는데 9·10월 편성표에서는
     그 자리가 **문법**이고 850 은 아예 없다. 850 을 평달에 여는지, 연다면 몇 시인지 받기 전까지 **850 반을 만들지 말 것.**
   - **방학달의 실제 시간대 값.** 담을 자리는 만들었지만(`season`) **행은 비어 있다.** 방학달은 해마다·달마다 달라서
     브로슈어 값을 넣어 두면 틀린다 — 같은 방학이어도 7월 850 은 80분 15:30~16:50, 8월 850 은 120분 12:30~14:40 이었고
     650 도 7월엔 12:30~14:40 이 있었지만 8월엔 없었다.
     그래서 **Alan 에게 그 해 그 달의 레벨별 실제 시간을 받아서 넣는다.** 받기 전에는 방학달 기수의 일괄 개설 표가 비고
     화면이 "아직 없음"을 알린다 (평달 시간대로 반이 만들어지지 않는다).
     넣는 방법도 같이 정할 것 — 지금은 `timetable_slots` 에 행을 넣는 화면이 없어 마이그레이션으로 넣어야 한다.
     매년 바뀌므로 **시간대 관리 화면**을 만들지 Alan 이 정한다.
   - **스파르타반·850 을 `courses` 에 어떻게 넣을지.** 스파르타는 `650LC/RC + 850LC/RC` 처럼 두 레벨이 묶여 있고
     시간도 180~240분이라 지금 강좌 모델(레벨 하나 + course_type 하나)에 그대로 들어가지 않는다.
