...
- **`/my` 대시보드 배치** (2026-09-22 Alan — "반갑습니다 알런님 이 문구를 맨 위에 한줄로 배치시키고 그 아래 부터 카드배치").
  **인사는 맨 위 한 줄**(`반갑습니다 이름 님`)이고 **카드는 그 아래부터**다 — 등록 현황 → 이번 달 내 시간표
  ...
  - **등록 현황 카드 왼쪽에 프로필 사진**을 둔다 (2026-09-22 Alan). **카카오·구글로 로그인한 학생은 그쪽 사진이
    ...
    - 받아 오지 못하면(CDN 404 · 주소 만료) `onError` 로 **이름 첫 글자 동그라미(`Avatar`)로 되돌린다** —
...
| 경로 | 내용 | 필요 등급 |
|---|---|---|
| `/my` | 대시보드. 맨 위 **인사 한 줄**, 그 아래 **내 등록 현황**(왼쪽에 프로필 사진 — 카카오·구글 사진이 자동으로), **이번 달 내 시간표**(달력 + 고른 날짜), **이번 기수 출석률**(현장 수강생), 등업신청 현황, 바로가기 | member |
