...
8. **YBM 수강후기 수집 + 특강 신청 조건** (2026-09-15 Alan 사전 공유. 아직 구현하지 않는다)
   Alan 이 하려는 것:
   - 서면 YBM 공식 페이지(`https://www.ybmedu.com/seomyon/winnertoeic`)의 역전토익 수강후기를 이 사이트로 가져온다.
     첫토익에서 이미 같은 방식으로 운영 중 — **평소 30분마다**, **특강 신청 기간에는 10분마다** 수집.
   - 후기의 **이름 · 등급 · 단계**를 우리 수강생과 대조해 "이 학생이 정말 후기를 썼는지" 판정한다.
   - 쓰임: **기수 마지막 3주차 특강**은 신청 조건이 *후기 작성*이다 (다른 특강은 조건 없음). 그 특강 신청 때 이 판정을 쓴다.

   구현 전에 확정해야 할 것:
   - **수집 허용 여부.** 네이버 예약에서 세운 원칙(약관 위반이면 긁지 않는다)과 같은 판단이 필요하다.
     YBM 공식 사이트 후기를 자동 수집해도 되는지 — 내부 승인이나 공식 제공 경로(API·피드·DB)가 있는지 먼저 확인한다.
   - **후기 페이지 원문 샘플.** 후기 목록 주소, 한 건에 보이는 항목, HTML 직접 렌더인지 JS/API 호출인지.
   - **매칭 키의 실제 값.** 후기에 이름이 마스킹되어 나오는지(김\*\*), "등급"·"단계"가 각각 무엇을 가리키는지
     (목표 점수반 650·750·850 / A·B반 / 기수 …). 동명이인 처리.
   - **매칭 실패 시 처리.** 수강증 OCR 처럼 스태프가 확인할지, 학생이 후기 주소를 직접 내게 할지.
   - **개인정보.** 남의 이름·후기 본문을 우리 DB 에 보관하게 된다. 보관 기간·마스킹 규칙을 수강증·네이버 예약과 같은 기준으로 정한다.
   - **후기를 어디에 보여줄지.** 지금 랜딩의 "누적 수강후기 7,356건" 은 `Stats.tsx`·`Marquee.tsx`·`Hero.tsx` 세 곳에 하드코딩되어 있다 (원칙 4 위반 상태).
   - **선행 조건: 특강 신청 기능이 아직 없다.** 특강은 달력 표시와 내 시간표 표시뿐이고 학생이 신청하는 흐름이 없다.
     신청 조건을 걸려면 신청 기능부터 만들어야 한다. 또 "마지막 3주차 특강"을 시스템이 알아야 하므로
     특강에 *후기 이벤트 대상* 표시가 필요하다 (지금 `kinds` 는 rc·lc·mock1·mock2 뿐).
   - **수집 주기.** "특강 신청 기간"의 정의. 분 단위 크론은 Vercel 요금제를 확인해야 한다
