...
- **관리자·강사·조교 모드 ↔ 학생 모드 전환** (2026-09-16 Alan 요청). 스태프와 조교에게만 보이는 두 칸짜리 토글로,
  ...
  PC(lg 이상)는 헤더 오른쪽에 두 칸 토글. **그보다 좁으면 햄버거 바로 왼쪽에 한 칸짜리 알약**을 늘 띄운다
  (2026-09-17 Alan 요청 "햄버거 메뉴 왼쪽에 … 바로 언제든지 스위칭이 가능하도록") — 글자는 **갈 곳**(`관리자`·`강사`·`조교`·`학생`)이고
  `aria-label` 에 "…모드로 바꾸기" 로 풀어 적는다. 두 칸 토글은 로고·햄버거와 한 줄에 놓기엔 넓어서 `variant="compact"` 를 따로 뒀다
  (320px 에서도 넘치지 않는 것을 확인했다). 햄버거 패널 맨 아래에도 두 칸 토글이 그대로 있다.
