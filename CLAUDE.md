...
- 등업신청에서 학생은 **수강증만 올린다.** 개월수·현장/불라방을 고르지 않는다
  (현장/불라방은 수강료로 판정 — 단 반에 수강료를 넣은 경우에만. 2026-09-16 부터 수강료는 선택이다).
  **금액이 없는 반에서는 이 규칙이 성립하지 않는다 — 미확정 2-1 이 정해지면 이 줄도 같이 고쳐야 한다.**
...
> ③ OCR 로 읽은 금액을 반 매칭이 아니라 현장/불라방 판정에만 쓴다
>
> **B. 현장/불라방 판정(`enrollments.mode`)은 근거 자체가 없어진다** — 점수가 낮아지는 게 아니라 판정이 불가능해진다. **미확정 2-1** 참고
> (미확정 2 는 "수강료를 넣지 않는다" 로 이미 해결된 항목이다. 헷갈리지 말 것).
>
...
2. ~~**수강료 실제 금액**~~ → **해결 (2026-09-16 Alan): 수강료는 넣지 않아도 된다.** 결제·등록은 YBM 에서만 하고
   ...
   금액을 임의로 지어내지 말 것.
2-1. **★ 현장/불라방(`enrollments.mode`)을 무엇으로 판정할지 — 새로 생긴 미확정.**
   도메인 규칙 4 는 "현장/불라방은 수강료로 판정", 확장 가정은 "수강료가 `tuition` 이면 onsite, `live_tuition` 이면 live" 인데,
   반에 금액이 없으면 **판정 근거가 통째로 사라진다.** 점수 기준선 조정(미확정 3)과는 **다른 결정**이므로 따로 정한다.
   영향: `/my/textbook`(불라방 수강생만 교재주문), 대시보드 시간대별 위젯 `현장 12 (불라방 5)`,
   그리고 "학생은 개월수·현장/불라방을 고르지 않는다"는 규칙 4 자체.
   선택지: (a) 등업신청에서 학생에게 현장/불라방 하나만 묻는다 — **규칙 4 를 고쳐야 한다** (b) 수강증 강좌명의 불라방 표시로 읽는다 — 실물 샘플 필요
   (c) 스태프가 학생명단에서 지정한다. **확정 전까지 어느 쪽도 구현하지 말 것.**
   **지금 당장은 실무 영향이 없다** — 승인 화면(`/admin/verifications/[id]`)에 현장/불라방 라디오가 이미 있고 나중에 바꾸는 select 도 있다. OCR 로 자동화할 때만 문제가 된다.
   같이 정할 것: 그 라디오가 지금 **현장으로 미리 선택**돼 있다(`defaultChecked`). 그냥 승인하면 불라방 학생이 조용히 onsite 로 박히고
   `/my/textbook` 이 막히는데(`queries.ts` 가 `mode = 'live'` 로 거른다) 화면에 아무 표시가 없어 알아채기 어렵다.
   **기본값을 없애고 스태프가 반드시 고르게 할지**를 Alan 이 정한다 (승인 1건마다 클릭이 하나 늘어난다).
...
- **현장/불라방 구분**: `enrollments.mode` (onsite | live). OCR 은 수강료가 `tuition` 이면 onsite, `live_tuition` 이면 live 로 판정.
  단 수강료는 선택이라 **반에 금액이 없으면 이 판정이 불가능하다** (2026-09-16). 그때 어떻게 할지는 **미확정 2-1** 에서 정한다
