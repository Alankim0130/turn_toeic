...
- **숙제업로드 — 정규 수업 숙제다** (2026-09-16 Alan 요청 → **2026-09-19 Alan 이 달력 방식으로 바꿨다**,
  ...
  - **`전체` · `모든 레벨` 칸은 없앴다 — 되살리지 말 것** (2026-09-22 Alan — "'전체' '모든레벨' 이 버튼은 없애줘").
    상태는 `점검 대기` · `점검 완료` 둘뿐이라 `전체` 는 이미 그 둘의 합이었고, 레벨 줄의 `모든 레벨` 은 칸 하나를 더 차지해
    줄마다 칸 수가 어긋났다. 지금은 **과목 3 · 레벨 3 · 상태 2** 라 세 줄의 끝이 맞는다.
    - **대신 레벨 칸마다 건수를 적는다** — `모든 레벨` 이 없어져 늘 한 레벨만 보고 있으므로, 650 을 보는 동안 750 에 숙제가
      쌓이면 **조용히 빠뜨린다** ("점검할 숙제가 없어요" 가 이 레벨에만 해당하는 말인데 전부 끝난 것처럼 읽힌다).
      숫자는 **그 칸을 누르면 보일 건수**다 (지금 과목·상태 그대로, 레벨만 바꿔서) — 상태 칸의 숫자와 같은 규칙이다.
    - 기본 레벨은 `lc_levels` 의 첫 줄이다. 레벨 목록을 못 읽으면 레벨로 거르지 않고 그 줄을 그리지 않는다.
    - **과목 줄의 `RC · LC 전체` 는 남긴다** — 관리자(알런)는 `profiles.subject` 가 비어 있어 기본으로 설 칸이 없고,
      두 과목을 한 번에 훑을 길이 사라진다. 그래서 과목 줄도 세 칸이 되어 레벨 줄과 끝이 맞는다
  - **사진은 팝업에서 넘겨 본다 — 링크로 되돌리지 말 것** (2026-09-22 Alan — "숙제 이미지를 클릭했을때 웹페이지가
    ...
    지금은 `HomeworkPhotos`(`src/components/admin/homework/`) 가 팝업을 띄우고 `n / N` · 화살표 · 손가락 넘기기 ·
    키보드 ← → 로 넘어간다. 수강후기 팝업(`Reviews`)과 **같은 규칙**이다 (ESC·배경·닫기로 닫고, 열려 있는 동안 뒤 화면 스크롤을 막는다).
    ...
    - 팝업은 **원본이 아니라 폭 1200 축소본**(`?w=1200`)을 받는다 — 원본은 장당 20MB 까지라 휴대폰에서 한 장마다 한참 기다린다.
    ...
    - **첨삭 도구(핀·스탬프·필기·지우개)는 만들지 않았다** — 첫토익 뷰어에는 있지만 채점·첨삭은
      ...
      **점검완료 버튼도 팝업에 두지 않는다** — 카드의 `HomeworkCheckForm` 한곳에서만 한다 (같은 일을 두 군데서 하지 않는다).
...
| `/admin/homework` | 숙제점검: **과목(= 강사, `RC · 이영수`) → 레벨(칸마다 건수) → 상태** 필터(강사는 자기 과목부터, `전체`·`모든 레벨` 칸은 없다), 최근순 목록, 학생 질문·수업 날짜 표시. **사진을 누르면 팝업에서 `n / N` 으로 넘겨 본다**(회전·원본 보기). 코멘트를 적고 점검완료하면 학생 알림함으로 간다 | instructor |
