...
- **강사 모드 ↔ 학생 모드 전환** (2026-09-16 Alan 요청). 스태프(instructor/admin)에게만 보이는 두 칸짜리 토글로,
  지금 있는 쪽이 분홍으로 차 있고 반대쪽을 누르면 넘어간다 (`학생 모드` → `/my`, `강사 모드` → `/admin`).
  PC 는 헤더 오른쪽, 그보다 좁으면 햄버거 패널 맨 아래. `src/components/layout/StaffModeSwitch.tsx` 한곳에서 관리한다.
  **모드는 저장하지 않고 주소로 정한다** — `/admin` 아래면 강사 모드, 그 밖은 학생 모드.
...
- 학생 화면: 내 시간표는 **내가 등록한 반(직접 배정)** 줄만 두고, 함께 열리는 반(시간 단위 · 스파르타가 여는 반)은 그 줄 아래 "함께 듣는 시간" 으로 붙인다
  — 120분 주5일 학생이 60분 줄 두 개를 보면 회차가 두 배로 보인다. 다시보기·불라방은 시간 단위 반마다 따로 나온다 (시간대를 함께 표시).
...
> 구현 메모(제안): `라이브방송`·`프리미어반`·`주5일`·`월수금`·`화목금` 도 G2 처럼 편집거리 ≤ 1 을 허용해야 한 글자 오인식에 무너지지 않는다.
...
-- timetable_slots(level, program, season, start_time, end_time) — 랜딩 시간표 · 일괄 개설 시간대. unique 는 다섯 칸 전부
...
| `/my/class` | 내 시간표 — 반 편성 달력의 수업일 + 그 달 특강·모의고사 + 특강 신청 바로가기 (주5일이면 두 트랙 합집합) | student |
...
| `/admin/sections` | 반 편성 달력(개강일·종강일·월수금·화목금·특강 → 항목별 저장 / 전체 저장, 이전/다음 달, 이미지 저장), 그 달 반 일괄 개설(강좌 × 시간대 × 트랙 표 → 고른 칸 한 번에) + 하나씩 만들기, 스터디 시간 설정 | instructor |
...
1. **시간대·반 목록** — 어떤 시간대에 어떤 반이 열리는지. Alan이 정리해서 전달 예정.
   ...
   **아직 확정 안 된 것**
   ...
   - **한 반이 트랙마다 과목이 다른 것을 어디에 적을지.** 650A 는 월수금 RC · 화목금 LC 인데
     `courses.course_type` 은 강좌당 하나라 트랙별 과목을 담을 자리가 없다. 지금은 화면에 과목을 따로 표시하지 않는다.
...
2-2. ~~**수강증 표기 규칙에서 남은 질문**~~ → **해결 (2026-09-16 Alan)**
   ...
   - ~~화목금 인강의 mode~~ → **현장(onsite)으로 둔다.** Alan: "월수금은 현장에 나와서 수업을 듣고, 화목금은 라이브방송을 보는 게 아니라
     ...
     **아직 구현 안 됨**: 저녁 화목금(인강) 반 학생에게 오전 화목금 반의 다시보기를 여는 규칙. 어느 오전 반인지는 편성표에서
     저녁 시간이 오전과 같은 과목 순서라(`18:30` 줄 = `10:00` 줄, `19:40` = `11:10`) 같은 강좌 · 같은 순서의 오전 반으로 짐작된다 — 구현 전에 확인.
     권한 판정은 `private.section_includes` 한곳에서 한다 (도메인 규칙 1 "반 권한").
