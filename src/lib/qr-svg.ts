/**
 * QR 칸 모양 → SVG path (2026-09-21). 교실 화면(`QrDisplay`)과 인쇄용 포스터(`attendance-poster.ts`)가 함께 쓴다.
 * 라이브러리가 만든 SVG 문자열을 innerHTML 로 넣지 않고 칸마다 네모를 그린다 — 가로로 이어진 칸은 한 조각으로 묶는다.
 * 순수 함수라 클라이언트 컴포넌트도 가져다 쓴다 (칸 모양을 만드는 `qrcode` 는 서버 쪽 `qr.ts` 에만 있다).
 */
