...
describe("실물 수강증 OCR 특성 (2026-09-18 역전토익 8월 수강증)", () => {
  it("`라이브방송` 이 줄바꿈으로 갈리고 사이에 라벨이 끼어도 불라방으로 읽는다", () => {
    // OCR 이 두 칸 배치를 읽은 순서 그대로: "…라이" 다음 줄에 "수강요일" 라벨, 그 다음 "브방송)"
    const p = parseReceipt(screen(["역전토익 [종합반]", "650 목표", "수강센터 부산 서면센터", "[4주-08/04] 주5일 (월18회 라이", "수강요일 .납'", "브방송)", "수강시간 10:00~12:10"].join("\n")));
    expect(p.mode).toBe("live");
    expect(p.weekly).toBe(5);
  });
