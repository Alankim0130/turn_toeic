...
export function decideVerification(parsed: ParsedReceipt | null, openTerms: readonly VerifyTerm[]): VerifyDecision {
  ...
  // 달을 읽었으면 그 달이 열린 기수에 있는지만 본다. 연도는 배지에 없으니 달만 맞춘다 (열린 기수는 이번 달·다음 달뿐이다).
  const courseMonth = receiptCourseMonth(parsed);
  if (openTerms.length > 0 && courseMonth != null) {
    if (!openTerms.some((t) => t.month === courseMonth)) {
      ...
      const ahead = (courseMonth - latest.month + 12) % 12; // 1~3 이면 다음 달(들), 그 밖은 지난달
      const isUpcoming = ahead >= 1 && ahead <= 3;
      return {
        kind: "reject",
        code: "month",
        reason: isUpcoming
          ? `이 수강증은 ${courseMonth}월 과정이에요. ${courseMonth}월 등업신청은 아직 열리지 않았어요 — 강사가 ${courseMonth}월 반을 열면 올릴 수 있어요 (보통 셋째 주부터, 동시등록 기간에는 더 일찍). 그때 다시 올려 주세요.`
          : `이 수강증은 ${courseMonth}월 과정(지난 달)이에요. 지금은 ${termsLabel(openTerms)} 등업신청만 받고 있어요. 이번 달 수강증을 올려 주세요.`,
      };
    }
    return { kind: "review" };
  }
